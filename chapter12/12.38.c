...
static int nthreads;
static sbuf_t sbuf; /* Shared buffer of conncted descriptors */
...
/*
 * creating thread that adjust total thread number according to sbuf situation
 * if sbuf is full, double threads
 * if sbuf is empty, half threads 
 */
...
void *adjust_threads(void *vargp) {
    ...
    while(1) {
        ...
        if(sbuf_empty(sp)) {
             ...
             * 
             * if you want to kill a thread, you must get the lock before it
             * so you won't kill a thread which is offering service.
             */
